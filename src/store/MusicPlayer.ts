...
export const MusicPlayer = defineStore('MusicPlayer', {
  state: () => {
    return {
      ...
      playlist: [] as realityPlaylistItemTypes[] | simplePlaylistItemTypes[],
  ...
  actions: {
    ...
    async getSongDetail(ids: number, isSingle: boolean = true) {
      ...
      } else {//如果是歌单id，则将请求到的songs数据需要插入到playlist的正确位置中
        this.playlist.splice(this.playlist.findIndex(item => item.id === ids), 1, ...res.data.songs)
      }
    ...
    async getPlaylistDetail(id: number) {
      ...
      (this.playlist as simplePlaylistItemTypes[]).unshift(...res.data.playlist.trackIds);
      //将歌单信息中的tracks（全部歌曲的详细信息）插入到playlist中,此时playlist的item为realityPlaylistItemTypes类型
      (this.playlist as realityPlaylistItemTypes[]).splice(0, res.data.playlist.tracks.length, ...res.data.playlist.tracks as unknown as realityPlaylistItemTypes[])
      this.playListMessageLoading = false;
    },
    //根据歌曲id获取歌曲url
    async getSongUrl(id: number) {
      ...
      (this.playlist as realityPlaylistItemTypes[])[this.playlist.findIndex(item => item.id === id)].url = res.data.data.map(item => ({ url: item.url, id: item.id }))[res.data.data.map(item => ({ url: item.url, id: item.id })).findIndex(item => item.id === id)].url
    }
  }
})